...
wss.on("connection", (ws, req) => {
    ...
    const client = { id, socket: ws, ip, alive: true };
    ...
    ws.on("message", (msg) => {
        ...
        try {
            ...
            console.error("[VIBE] Message error:", e);
        }
    });
    ws.on("close", () => {
        console.log(`[VIBE] Ext ${id} disconnected`);
        clients.delete(id);
        for (const [sid, eid] of pairings.entries())
            if (eid === id)
                pairings.delete(sid);
    });
});
...
function createMCPServer(sseSessionId) {
    ...
    server.setRequestHandler(CallToolRequestSchema, async (req) => {
        ...
        if (!client)
            throw new Error("No paired wallet found on this machine. Ensure your browser extension is open.");
        return new Promise((resolve, reject) => {
            ...
            pendingRequests.set(id, {
                resolve: (r) => resolve({ content: [{ type: "text", text: typeof r === 'string' ? r : JSON.stringify(r) }] }),
                reject: (e) => reject(e),
                timeout
            });
...
app.get("/sse", async (req, res) => {
    ...
    if (local)
        pairings.set(sid, local.id);
    const server = createMCPServer(sid);
    res.on("close", () => { sseSessions.delete(sid); pairings.delete(sid); });
    await server.connect(transport);
});
app.post("/sse", (req, res) => {
    const s = sseSessions.get(req.query.sessionId);
    if (s)
        s.sse.handlePostMessage(req, res);
    else
        res.status(404).end();
});
